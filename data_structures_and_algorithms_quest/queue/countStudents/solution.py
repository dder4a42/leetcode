...
class Solution:
    def solve(self, input_data: str) -> str:
        ...
        students, sandwiches = input_data.split(" ")
        students = [ int(x) for x in students.split(",")]
        sandwiches = [ int(x) for x in sandwiches.split(",")]
        ...
        #         else:
        #             break
        #     if len(dequeue) == len(students):
        #         return len(students)
        #     students = students[len(dequeue)+1:] + dequeue
        # return 0

        # version 2, every sandwich in the top will tried by all students, so just cnt distribution of stu
        cnt = [0, 0]
        for s in students:
            cnt[s] += 1
        for s in sandwiches:
